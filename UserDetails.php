...
<!------------------------> 
    ...
    <br/>
       <h3>User Details - تفاصيل المستخدم</h3>
       ...
       <div class="row align-items-center justify-content-center">
             ...
             <button class="btn btn-info m-4 w-20-300" data-toggle="modal" data-target="#ResetPasswordModal"><i class="fa-solid fa-lock"></i><br/>Reset Password <br/> تغيير كلمة المرور</button>
           
             <?php include("./Modals/ResetPasswordModal.php"); ?>
       

             <button class="btn btn-info m-4 w-20-300" data-toggle="modal" data-target="#UserAddCarAccidentModal"><i class="fa-solid fa-car-crash"></i><br/>Accident<br/>  حادث </button> 
             <?php include("./Modals/UserAddCarAccidentModal.php"); ?>
             

             <button class="btn btn-info m-4 w-20-300" data-toggle="modal" data-target="#NewRequestForCar"><i class="fa-solid fa-car"></i> <i class="fa-solid fa-screwdriver-wrench"></i> <br/>New Request    <br/>    طلب جديد  </button>
         ...
         <?php require("Modals/NewRequestForCar.php") ?>
           ...
           <table class="  table-hover carDataTable">
              <thead>
                  <tr>
                      <td colspan="11" class="bg-dark">
                        <h4>Cars under This user - مركبات تحت أسم المستخدم</h4>
                      </td>
                  </tr>
                  <tr>
                      ...
                      <td class='hidecolumns'>Year Of Make <br/> سنة الصنع</td>
                      <td> Plate Number <br/> رقم اللوحة </td>
                      <td class='hidecolumns'> VID <br/> رقم الهيكل </td>
                      <td class='hidecolumns'> Color <br/> اللون </td>
                      <td>Date <br/> تاريخ الإستلام </td>
                      <td>Return Date <br/> تاريخ التسليم </td>
              ...
              $sql = "select `car_id`, `received_date`, `handover_date`  from `car_users` where `driver_id` =".$_GET["id"];
              $result = $conn->query($sql);
              $count =1;
              while($row = $result->fetch_array(MYSQLI_ASSOC))
              {
                  ...
                  echo(' <td ><a href="carDetails.php?id='.$row["car_id"].'" class="btn btn-info w-100">'.$row["car_id"].'</a></td>');
                  echo("<td>".$row2["model"]."</td>");
                  echo("<td class='hidecolumns'>".$row2["brand"]."</td>");
                  echo("<td class='hidecolumns'>".$row2["year_make"]."</td>");
                  echo("<td>".$row2["plate_number"]."</td>");
                  echo("<td class='hidecolumns'>".$row2["vid"]."</td>");
                  echo("<td class='hidecolumns'>".$row2["car_color"]."</td>");
                  echo("<td>".$row["received_date"]."</td>");
                  echo("<td>".$row["handover_date"]."</td>");
                  echo("<td class='hidecolumns'>".$row2["car_status"]."</td>");
                  echo("</tr>");
                  $count++;
              }
              ?>
         ...
         <table class="  table-hover carDataTable">
              <thead>
                  <tr>
                      <td colspan="11" class="bg-dark">
                        <h4>User Request -    الطلبات</h4>
                      </td>
                  </tr>
                  <tr>
                      <td>SN.</td>
                      <td>ID</td>
                      <td>Request <br/>الطلب</td>
                    
                      <td class="w-50">Justification <br/>  التبرير</td>
              ...
              <tbody>
            <?php
              $sql = "select `id`,  `request_type`  ,`justification` ,`request_date`,`transportation_response` from `requestes_for_cars` where `user_id` =".$_GET["id"];
              $result = $conn->query($sql);
              $count =1;
              while($row = $result->fetch_array(MYSQLI_ASSOC))
              {
         ...
         <div class="row m-3">
            ...
            <button class="btn btn-success m-1" onclick="editUser()" <?=$disabledInput?>><i class="fa-solid fa-floppy-disk"></i> <br/>update <br/> تعديل</button>
