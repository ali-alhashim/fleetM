...
<button class="btn btn-info m-2 w-20-300" data-toggle="modal" data-target="#PaymentReceiptModal">
     <i class="fa-solid fa-hand-holding-dollar"></i><br/> Add Payment receipt     
</button>  


<?php include("Modals/PaymentReceiptModal.php"); ?>
         ...
         <!-- Car Data Table-->
          ...
          <table class="  table-hover carDataTable">
              ...
              <?php
                 ...
                 $sql = "select * from `payment_receipt` $SQLSearch  order by `id` DESC $limit ; ";
                 ...
                 while($row = $result->fetch_array(MYSQLI_ASSOC))
                 {
                     ...
                     echo("<td><a href='PaymentDetails.php?id=".$row["id"]."' class='btn btn-info w-100'>".$row["id"]."</td>");
                     echo('<td >"'.$row["pay_to"].'</td>');
                     echo('<td >'.$row["pay_from"].'</td>');
                     echo("<td>".$row["payment_method"]."</td>");
                    
                     echo("<td>".$row["total_invoices"]."</td>");
                     echo("<td>".$row["note"]."</td>");
                    
                     echo("<td>".$row["payment_amount"]."</td>");
                     echo("<td><a href='".$row["document"]."' target='_blank'>open<a/></td>");
